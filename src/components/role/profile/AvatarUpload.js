...
export default function AvatarUpload({
  ...
  const fileInputRef = useRef(null); // 既にアイコンがある場合はファイル選択を強制しない
    ...
    <div>
      ...
      <section className="py-16">
        ...
        <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="bg-white border border-gray-200 rounded-2xl p-8 shadow-sm flex flex-col items-center gap-8 mb-10">
            <input
              id="name"
              name="name"
              type="text"
              className="w-full px-4 shadow-sm py-3 border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-black"
              value={name}
              onChange={(e) => setName(e.target.value)}
            />
          </div>

          <label htmlFor="icon" className="block text-sm font-medium mb-2">プロフィール画像</label>
